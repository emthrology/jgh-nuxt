<template>
  <div class="mt-[85px] mb-[120px] flex flex-col items-center">
    ...
    <div
      class="mt-4 pb-4 w-[90%] rounded-2xl bg-zinc-200 flex flex-col items-center"
    >
      <Calendar
        :schedules="schedules"
        @select-date="changeSelectDate"
...
//   ? data.value.map((schedule) => schedule.start_at)
//   : [];
// schedules.value = scheduleDays;
const selectedSchedule = ref([]);

const selectedDay = ref(dayjs(new Date()).format('YYYY-MM-DD'));
const changeSelectDate = (date) => {
  ...
  const selectedScheduleInfo = data.value.filter((schedule) =>
    schedule.start_at.includes(forrmattedDate)
  );
